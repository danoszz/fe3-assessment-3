(function () {
  ...
  d3.json(
    '../assets/data/aframe-data-bloemendaal_small.json',
    (error, data) => {
      global.data = data;
      global.number_of_lampposts = data.length;

      // Set some consts for later use
      const height_min_max = d3.extent(data, (d) => {
      ...
      const year_min_max = d3.extent(data, (d) => {
        if (d !== undefined) {
          return d.Aanlegjaar;
        }
      });
      ...
      const y_scale = d3
        ...
        .domain([0, height_min_max[1]])
        .range([0, height_min_max[1] * 2]); // takes longest object and doubles the space
      ...
      const z_scale = d3
        ...
        .range([100, 0]); // should be opposite to x-scale, but same number for correct representation

      const year_scale = d3
        .scaleLinear() // Create a linear scale just for the years
        .domain([year_min_max[0], year_min_max[1]])
        .range([0, 2]);
      ...
      global.percentile_scale = d3
        .scaleLinear()
        .domain([height_min_max[0], median, 20])
        .range([1, 70, 140])
        .clamp(true);
      ...
      global.color_scale = d3
        .scaleLinear()
        .domain([height_min_max[0], median])
        .range(['#FFAFA2', '#813BF6'])
        .clamp(true);

      // Create for each object a a-frame item in the a-scene
      data.forEach((d, i) => {
        const scene = d3.select('a-scene');

        // Use d3's enter/update/exit pattern to draw and bind our dom elements
        const lantarns = scene
          .selectAll('a-cylinder.lantarn')
          .data(data)
          .enter()
          .append('a-cylinder')
          .classed('lantarn', true)
          .attr('position', (d, i) => {
            let x = x_scale(d.longitude);
            let z = z_scale(d.latitude);
            let y = y_scale(d.Hoogte) / 2;

            return x, y, z; // Strings are not allowed for DOM manipulation, https://github.com/aframevr/aframe/pull/1323
          })
          .attr('height', (d, i) => {
            let h = y_scale(d.Hoogte);
            return Number(h);
          })
          .attr('radius', (d, i) => {
            let ye = year_scale(d.Aanlegjaar);
            return Number(ye);
          })
          .append('a-animation')
          .attr('begin', '1000')
      ...
      d3.selectAll('.lantarn').on('click', (d, i) => {
        updateCardText(d, i);
  ...
  d3.json('../assets/data/contour-data.json', (error, data) => {
    ...
    // const contourList = c.contourList().sort(
    //   (a, b) =>
    //     d3.min(a.map((d) => {
		// 				return d.x;
		// 			}),) -
